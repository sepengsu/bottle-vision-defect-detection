...
# =================== GUI 초기화 ===================
root = Tk()
root.title("Vision System (Reverse Sequence)")
...
try:
    ...
    if len(devices) == 0:
        raise IOError("Basler 카메라가 발견되지 않았습니다.")
    ...
    messagebox.showerror("Error", f"카메라 초기화 실패: {e}")
    root.destroy()
    exit()
            ...
            with frame_lock:
                for cam_id in sorted(TARGET_CAMS):
                    if cam_id in latest_frames:
                        ...
                        cv2.putText(black_img, f"CAM {cam_id} Off", (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (100, 100, 100), 2)
